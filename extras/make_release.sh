...
cd extras
echo "python update_json_data.py -a "msp430" -v ${ENERGIA1_VER}  -n "msp430-gcc" -c ${LEGACY_GCC_VER} -d ${DSLITE_VER} -e ${MSPDEBUG_VER} -i ${INO2CPP_VER} -u ${ENERGIA_URL} -t ${TOOLS_URL} -f package_index.json.template"
python update_json_data.py -a "msp430" -v ${ENERGIA1_VER}  -n "msp430-gcc" -c ${LEGACY_GCC_VER} -d ${DSLITE_VER} -e ${MSPDEBUG_VER} -i ${INO2CPP_VER} -u ${ENERGIA_URL} -t ${TOOLS_URL} -f package_index.json.template
cd ..
